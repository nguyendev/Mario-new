...
void Mario::Update(list<BaseObject*>*	_staticObjs, list<BaseObject*>*	_dynamicObjs, Input* input, float TPF)
{
	ProcessInput(input);
	Move(TPF);
...
void Mario::UpdateAnimation(float TPF)
{
	...
	else if (isJumping == false)
	{
		...
		{
			_ani->Update(3, 5, TPF);
			if (isChangingDirectionToLeft&&isChangingDirectionToRight == false)
				_ani->SetIndex(2);
			else if (isChangingDirectionToLeft == false && isChangingDirectionToRight)
				_ani->SetIndex(2);
		}
...
{
	if (isRight)
		_sprite->Draw(_x, _y, ZOOM_MARIO, ZOOM_MARIO, _ani->GetRect(_ani->index), DEEP_MARIO);
	else
		_sprite->Draw(_x, _y, -ZOOM_MARIO, ZOOM_MARIO, _ani->GetRect(_ani->index), DEEP_MARIO);
}
