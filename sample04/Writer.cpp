...
void ReadMap(char* fileName, bool isBright, CGameMario* game)
{
	...
	for (int i = 0; i < _count; i++)
	{
		...
		case 50:
			...
			break;
		case 51:
			obj = new MuchMoneyBrick(PIXEL * (t[i].srcX), PIXEL * (t[i].srcY), _camera->_cameraX, _camera->_cameraY, t[i].id, game->_sprites[S_BRICK], 0);
			_isStatic = true;
			break;
		case 52:
			obj = new SpecialBrick(PIXEL * (t[i].srcX), PIXEL * (t[i].srcY), _camera->_cameraX, _camera->_cameraY, t[i].id, game->_sprites[S_BRICK], 0);
			_isStatic = true;
			break;
			/*default:
