...
SpecialBrick::SpecialBrick(float x, float y, float _cameraX, float _cameraY, int ID, CSprite* sprite, int SpriteIndex)
{
	_sprite = sprite;
	_ID = ID;
	_isChanged = false;
...
void SpecialBrick::Update(float TPF, list<BaseObject*>* staticObj, list<BaseObject*>* dynamicObj, KeyBoard* keyboard)
{
	...
	{
	case TS_IDLE:
		_sprite->setIndex(0);						// đang chờ thì trông giống gạch thường
		break;
	case TS_MOVEUP:
